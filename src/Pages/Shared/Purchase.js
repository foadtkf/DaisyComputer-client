...
const Purchase = () => {
  ...
  const url = `https://daisycomputer-server.onrender.com/products/${id}`;
  const {
    data: product,
    isLoading,
    refetch,
  ...
  const handleButton = (event) => {
    ...
    if (
      ...
      parseInt(purchaseQuantity) <= quantity
    ) {
      setError("");
      setBtnDisable(false);
    } else {
    ...
    fetch(url, {
      ...
      .then((data) => {
        if (data.modifiedCount) {
          refetch();
          fetch(`https://daisycomputer-server.onrender.com/booking`, {
            method: "POST",
            headers: {
              "content-type": "application/json",
              authorization: `Bearer ${localStorage.getItem("accessToken")}`,
              ...
              <div>
                ...
                <div className="bg-transparent mx-auto rounded-2xl w-100 ">
                  <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <input
                      disabled={btnDisable}
                      className="btn w-full max-w-xs my-5 text-white"
                      type="submit"
                      value="Place Order"
                    />
                  </form>
